---
name: repair-ui-copy
description: Use when a Vue or dashboard UI shows garbled Chinese copy, literal question-mark placeholders, broken toast text, encoding damage after shell edits, or long labels that overflow cards, dialogs, or badges.
---
...
## Quick Reference
- Search for literal `???` and broken placeholder strings first.
...
## Workflow
1. Scan the affected files for `???`, mojibake, and visibly corrupted strings.
...
## Example
Use when the user says: ??????????, ???? toast ????, or ??????????????.
